...
func (h *mqHandler) GetDescription() string {
	if h.HasConsumedMQ() {
		return fmt.Sprintf("接口名称： %s\n\n接口描述： 本文档描述该接口的出入参数，该HTTP请求模式仅为方便单用例测试用，实际接入工作建议以消息队列接入形式。\n\n消息队列： %s\n\n消息路由键： %s\n", h.Name, h.MQTopic, h.RoutingKey)
	}
	return fmt.Sprintf("接口名称： %s\n", h.Name)
}
...
func (h *mqHandler) GetOperationID() string {
	if "POST" == h.AllowsMethod {
		return h.Name
	}
	return h.Name + "." + strings.ToLower(h.AllowsMethod)
}

// GetAllowsMethod for restful handler method
func (h *mqHandler) GetAllowsMethod() string {
	return h.AllowsMethod
}
...
func (h *mqHandler) GetRequestBodyDocsInfo() autodocs.RequestBodyInfo {
	...
	if h.HasConsumedMQ() {
		requestInfo.Description = requestInfo.Description + "\n本文档描述该接口的出入参数，该HTTP请求模式仅为方便单用例测试用，实际接入工作建议以消息队列接入形式。"
	}
...
// GetController of hander
func (h *mqHandler) GetController() Controller {
	return h.controller
}

// IsDisablesAuthorization bool
func (h *mqHandler) IsDisablesAuthorization() bool {
	if nil != h.controller {
		return h.controller.IsDisablesAuthorization()
	}
	return false
}

// HasConsumedMQ by controller
func (h *mqHandler) HasConsumedMQ() bool {
	if nil == h.controller {
		return false
	}
	return h.controller.HasConsumedMQ()
}
...
func (hm *mqHandlers) RegisterHandlers(topicCategory string, hs map[string]MQHandler) error {
	...
	for rk, h := range hs {
		h.SetMQTopic(topic)
		h.GetController().SetConsumedMQ(consumedMQ)
		hm.handlers[rk] = h
	}
	hm.m.Unlock()
	return nil
...
func (hm *mqHandlers) GetHandler(routingKey string) resthandlers.Handler {
	hm.m.RLock()
	var h resthandlers.Handler
	if nil != hm.handlers {
...
func (hm *mqHandlers) AllDocHandlers() []autodocs.DocsHandler {
	hm.m.RLock()
	l := len(hm.handlers)
	handlers := make([]autodocs.DocsHandler, l)
	if nil != hm.handlers {
		i := 0
...
func analyzeControllerHandlers(controller Controller) map[string]MQHandler {
	...
	for i := 0; i < ml; i++ {
		rmt := rtv.Method(i)
		if SkipControllerHanderNames[rmt.Name] {
			continue
		}
		handlerPieces := utils.CamelCaseSlices(rmt.Name)
		handlerMethod := HandlerPrefix[handlerPieces[0]]
		if "" != handlerMethod {
			handlerName := utils.KebabCaseString(rmt.Name[len(handlerPieces[0]):])
			logger.Debug.Printf("analyzing %s method %s", controllerName, rmt.Name)
			if "" == handlerName {
				handlerName = strings.ToLower(handlerMethod)
			}
			rmv := rv.Method(i)
			...
			h := &mqHandler{
				Name:         handlerName,
				Group:        handleGroup,
				RoutingKey:   fmt.Sprintf("%s.%s", handleGroup, handlerName),
				AllowsMethod: handlerMethod,
				caller:       rmv,
				inTypes:      make([]paramTypeWrapper, numIn),
				outTypes:     make([]paramTypeWrapper, numOut),
				controller:   controller,
			}
			switch handlerPieces[0] {
			case "Handler", "Handle":
				break
			default:
				h.RoutingKey = fmt.Sprintf("%s.%s", h.RoutingKey, strings.ToLower(handlerMethod))
				break
			}
...
func initializeControllerFilters(controller Controller, initNamePrefix string) {
...
func initializeControllerVisitors(controller Controller, initNamePrefix string) {
...
func initializeControllerObservers(controller Controller, initNamePrefix string) {
...
func initializeControllerDelegates(controller Controller, initNamePrefix string) {
